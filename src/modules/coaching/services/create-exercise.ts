...
import { createExerciseRecord } from "@/modules/coaching/services/exercise-service";
...
export async function createExercise(
  ...
  const { data, error } = await createExerciseRecord(
    supabase,
  ...
  if (error || !data) {
    return {
      error: error?.message ?? "Unable to create exercise.",
    };
  }
