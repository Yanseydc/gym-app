...
export async function createProgressCheckIn(
  ...
  const { data, error } = await createProgressCheckInRecord(
    supabase,
    clientId,
  ...
  if (error || !data) {
    return {
      error: error?.message ?? "Unable to create progress check-in.",
    };
  }
  ...
  redirect(`/dashboard/clients/${clientId}/progress-checkins/${data.id}/edit`);
}
