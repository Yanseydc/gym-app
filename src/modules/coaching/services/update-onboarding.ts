...
import { updateOnboardingRecord } from "@/modules/coaching/services/onboarding-service";
import type {
...
export async function updateOnboarding(
  ...
  const { data, error } = await updateOnboardingRecord(
    supabase,
    clientId,
  ...
  if (error || !data) {
    return {
      error: error?.message ?? "Unable to update onboarding.",
    };
  }
